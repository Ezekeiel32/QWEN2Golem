...
#!/usr/bin/env python3
"""
Enhanced Aether Memory Integration System - FIXED VERSION
Automatically integrates ALL collections including PKL files into the golem's memory bank
"""
...
class EnhancedAetherMemoryLoader:
    """Enhanced loader for all aether collections with intelligent integration - FIXED"""
    ...
    def auto_discover_aether_files(self) -> List[Dict[str, Any]]:
        """Automatically discover all aether-related JSON AND PKL files from specified directories."""
        # Directories to scan for memory files.
        scan_dirs = ["/home/chezy", "."]
        aether_files = []

        for current_dir in scan_dirs:
            if not os.path.isdir(current_dir):
                self._log(f"⚠️  Directory not found, skipping: {current_dir}")
                continue

            self._log(f"🔎 Scanning directory: {current_dir}")

            # Scan for both JSON and PKL files
            for filename in os.listdir(current_dir):
                ...
                # Check PKL files
                    ...
                    'aether', 'golem', 'memory', 'corpus', 'training'
                ]):
                    is_aether_file = True
                    file_type = 'pkl'
        ...
        self._log(f"🔍 Discovered {len(aether_files)} aether files across all scanned directories:")
        for file_info in aether_files:
            self._log(f"   📂 {file_info['path']} ({file_info['size_kb']:.1f} KB) [{file_info['type'].upper()}]")
    ...
    def _calculate_priority(self, filename: str, file_size: int, file_type: str) -> float:
        ...
        # Name-based priority
        if 'aether_corpus_memory' in filename.lower():
            priority += 3000  # Highest priority
        elif 'golem_aether_memory' in filename.lower():
            priority += 2500
        elif 'golem_aether_training' in filename.lower():
            priority += 2000
        elif 'real_aether_collection' in filename.lower():
            priority += 1500
        elif 'enhanced_aether_memory_bank' in filename.lower():
            priority += 1000
        elif 'optimized' in filename.lower():
            priority += 500
        elif 'checkpoint' in filename.lower():
            priority += 300
    ...
    def _load_pkl_file(self, filepath: str, filename: str) -> List[Dict[str, Any]]:
        """Load patterns from PKL file - THIS WAS COMPLETELY MISSING"""
            ...
            if 'memories' in data:
                for memory in data['memories']:
                    pattern = {
                        ...
                        'aether_epsilon': memory.get('cycle_params', {}).get('aether_epsilon', 0),
...
def main():
    ...
    print("="*60)
    print("AETHER MEMORY INTEGRATION UTILITY - FIXED VERSION")
    print("="*60)
